...
set -e
[[ "${DEBUG}" == "true" ]] && set -x
: ${CONSUL_DATA_DIR:=/consul/data} ${CONSUL_CONFIG_DIR=/consul/config}
...
if [ -n "$CONSUL_BIND_INTERFACE" ]; then
  CONSUL_BIND_ADDRESS=$(ip -o -4 addr list $CONSUL_BIND_INTERFACE | awk -F'[ /]+' '{print $4;exit}' )
  if [ -z "$CONSUL_BIND_ADDRESS" ]; then
    echo "Could not find IP for interface '$CONSUL_BIND_INTERFACE', try to use the default route NIC"
    CONSUL_BIND_INTERFACE=$(ip route | awk '/default/{print $NF}')
    CONSUL_BIND_ADDRESS=$(ip -o -4 addr list $CONSUL_BIND_INTERFACE | awk -F'[ /]+' '{print $4;exit}' )
    if [ -z "$CONSUL_CLIENT_ADDRESS" ]; then
      echo "Could not find IP, exiting"
      exit 1
    fi
  fi
...
if [ -n "$CONSUL_CLIENT_INTERFACE" ]; then
  CONSUL_CLIENT_ADDRESS=$(ip -o -4 addr list $CONSUL_CLIENT_INTERFACE | awk -F'[ /]+' '{print $4;exit}' )
  if [ -z "$CONSUL_CLIENT_ADDRESS" ]; then
...
if [ "$1" = 'agent' ]; then
    shift
    if ! echo $@ | grep -q '-retry-join' && [ -n "${RETRY_JOIN}" ] ;then
      set -- $@ $(echo $RETRY_JOIN | tr , '\n' | sed -r 's#.+#-retry-join=&#' | paste -d ' ' -s -)
    fi
    set -- consul agent \
